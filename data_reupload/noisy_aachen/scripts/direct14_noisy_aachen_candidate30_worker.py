...
def make_seasonal_windows(
    ...
    """
    Construct the same supervised train/validation/test split as make_windows(),
    but encode every input lag as:

        [theta_cdf, month_index]

    The empirical CDF is frozen using the training reference observations

        0, ..., window_size + train_window_end - 1.
    ...
    so the reference is observations 0,...,363 inclusive (364 values).

    The target remains in the fixed [-1, 1] scaling used by the original
    noisy forecasting experiment.
    """
    ...
    if (
        cdf_reference_end <= 0
        or
        cdf_reference_end > len(y_original)
    ):
        raise ValueError(
            "Invalid seasonal CDF reference endpoint: "
            f"{cdf_reference_end}"
        )
    ...
    for i in range(
        ...
        # Exact seasonal CDF polar transformation used
        # in the validated ideal models.
        theta_cdf = np.arccos(z)

        # Absolute source-time indices of all lags.
...
def parse_initial_layout(
    value: Optional[Any],
    *,
    n_qubits: int,
    backend_n_qubits: int,
) -> Optional[Tuple[int, ...]]:
    """Parse and validate a logical-to-physical initial layout."""
    if value is None:
        return None
    ...
    if isinstance(value, str):
        ...
        try:
            layout = tuple(int(token) for token in tokens)
        except ValueError as exc:
            raise ValueError(
                "--initial_layout must be a comma-separated list of integers."
            ) from exc
    else:
        layout = tuple(int(item) for item in value)

    if len(layout) != int(n_qubits):
        raise ValueError(
            f"Initial layout contains {len(layout)} physical qubits; "
            f"exactly {n_qubits} are required."
        )
    if len(set(layout)) != len(layout):
        raise ValueError(f"Initial layout contains duplicate qubits: {layout}")
    if min(layout) < 0 or max(layout) >= int(backend_n_qubits):
        raise ValueError(
            f"Initial layout {layout} must lie within physical-qubit range "
            f"[0, {backend_n_qubits - 1}]."
        )

    return layout


def load_requested_fake_backend(
    *,
    fake_provider: Any,
    n_qubits: int,
    ...
    """Load either an installed fake backend or the frozen Aachen snapshot."""
    if snapshot_dir is None:
        return select_fake_backend(
            fake_provider=fake_provider,
            n_qubits=n_qubits,
            class_name=class_name,
        )

    if class_name not in {None, "FakeAachenSnapshot"}:
        raise ValueError(
            "A frozen snapshot directory currently supports only "
            "--fake_backend_class FakeAachenSnapshot."
        )

    snapshot_path = Path(snapshot_dir).expanduser().resolve()
    required_files = (
        snapshot_path / "conf_aachen.json",
        snapshot_path / "props_aachen.json",
    )
    missing_files = [str(path) for path in required_files if not path.is_file()]
    if missing_files:
        raise FileNotFoundError(
            "FakeAachen snapshot is incomplete; missing: " + ", ".join(missing_files)
        )

    from qiskit_ibm_runtime.fake_provider import fake_backend as fake_backend_base

    class FakeAachenSnapshot(fake_backend_base.FakeBackendV2):
        """Fake Aachen backend loaded from the frozen local snapshot."""

        dirname = str(snapshot_path)
        conf_filename = "conf_aachen.json"
        props_filename = "props_aachen.json"
        backend_name = "fake_aachen"

    backend = FakeAachenSnapshot()
    backend_n = int(backend.num_qubits)
    if backend_n < int(n_qubits):
        raise ValueError(
            f"FakeAachenSnapshot has {backend_n} qubits but {n_qubits} are required."
        )

    return "FakeAachenSnapshot", backend, backend_n


def validate_chain_layout(fake_backend: Any, layout: Sequence[int]) -> int:
    """Verify that consecutive entries form a physical chain."""
    coupling_map = fake_backend.target.build_coupling_map()
    if coupling_map is None:
        raise RuntimeError("The selected fake backend does not expose a coupling map.")

    coupling_edges = {tuple(edge) for edge in coupling_map.get_edges()}
    missing_edges = []
    for first_qubit, second_qubit in zip(layout[:-1], layout[1:]):
        if (
            (first_qubit, second_qubit) not in coupling_edges
            and (second_qubit, first_qubit) not in coupling_edges
        ):
            missing_edges.append((int(first_qubit), int(second_qubit)))

    if missing_edges:
        print(
            "INFO: initial layout contains non-direct consecutive pairs; "
            f"SABRE will route them: {missing_edges}"
        )

    return len(coupling_edges)


def install_topology_compile_hook(
    device: Any,
    *,
    routing_method: str,
    initial_layout: Sequence[int],
    expected_logical_entanglers: int,
    metadata_path: Optional[str],
):
    """Replace the legacy plugin's routing_method='none' compile step."""
    from qiskit.compiler import transpile as qiskit_transpile

    # qml.device() wraps legacy PennyLane devices in LegacyDeviceFacade.
    # Execution calls batch_execute() on the wrapped target_device, so an
    # instance method attached only to the facade is never used. Resolve the
    # actual Qiskit legacy device before installing the routing override.
    compile_device = getattr(device, "target_device", None)
    if compile_device is None:
        compile_device = getattr(device, "_device", None)
    if compile_device is None:
        compile_device = device

    if not hasattr(compile_device, "compile"):
        raise RuntimeError(
            "Could not locate the legacy Qiskit device compile method behind "
            f"{type(device).__module__}.{type(device).__name__}."
        )
    if not hasattr(compile_device, "transpile_args"):
        raise RuntimeError(
            "The resolved legacy Qiskit device does not expose transpile_args: "
            f"{type(compile_device).__module__}.{type(compile_device).__name__}."
        )

    compile_device.transpile_args["initial_layout"] = list(initial_layout)

    def compile_with_topology(self):
        transpile_args = dict(self.transpile_args)
        transpile_args.pop("routing_method", None)
        ...
        if not getattr(self, "_topology_metadata_recorded", False):
            compiled_counts = {
                str(name): int(count)
                for name, count in compiled_circuit.count_ops().items()
            }
            logical_counts = {
                str(name): int(count)
                for name, count in self._circuit.count_ops().items()
            }
            compiled_cz_count = int(compiled_counts.get("cz", 0))

            # ------------------------------------------------------------
            # Active physical-qubit / routed-edge diagnostics
            # ------------------------------------------------------------
            # Qiskit pads the transpiled circuit to the full backend width.
            # compiled_circuit.num_qubits therefore does NOT indicate the
            # number of physical qubits actually touched by the computation.
            #
            # Record only qubits that participate in nontrivial instructions,
            # and separately record all two-qubit physical edges.
            active_physical_qubits = set()
            active_two_qubit_edges = []
            active_two_qubit_edge_counts = {}

            for instruction in compiled_circuit.data:
                operation = instruction.operation
                qargs = instruction.qubits

                # Ignore barriers and measurements when determining the
                # unitary/noisy simulation footprint.
                if operation.name in {"barrier", "measure"}:
                    continue

                physical_indices = [
                    int(compiled_circuit.find_bit(q).index)
                    for q in qargs
                ]

                for physical_index in physical_indices:
                    active_physical_qubits.add(physical_index)

                if len(physical_indices) == 2:
                    edge = tuple(physical_indices)
                    active_two_qubit_edges.append(
                        [int(edge[0]), int(edge[1])]
                    )

                    edge_key = f"{edge[0]}->{edge[1]}"
                    active_two_qubit_edge_counts[edge_key] = (
                        active_two_qubit_edge_counts.get(edge_key, 0) + 1
                    )

            active_physical_qubits = sorted(active_physical_qubits)

            payload = {
                "routing_method": str(routing_method),
                "initial_layout": [int(qubit) for qubit in initial_layout],
                "expected_logical_entanglers": int(expected_logical_entanglers),
                "compiled_cz_count": compiled_cz_count,
                "additional_cz_over_logical_entanglers": (
                    compiled_cz_count - int(expected_logical_entanglers)
                ),
                "logical_operation_counts": logical_counts,
                "compiled_operation_counts": compiled_counts,
                "compiled_depth": int(compiled_circuit.depth()),
                "compiled_size": int(compiled_circuit.size()),
                "compiled_width": int(compiled_circuit.num_qubits),
                "active_physical_qubits": active_physical_qubits,
                "active_physical_qubit_count": len(active_physical_qubits),
                "active_two_qubit_edges": active_two_qubit_edges,
                "active_two_qubit_edge_counts": active_two_qubit_edge_counts,
                "qubit_30_active": 30 in active_physical_qubits,
                "transpile_layout_repr": str(compiled_circuit.layout),
            }

            if metadata_path:
                output_path = Path(metadata_path).expanduser().resolve()
            ...
            print(
                "TOPOLOGY_TRANSPILE_METADATA="
                + json.dumps(payload, sort_keys=True),
                flush=True,
            )
            self._topology_metadata_recorded = True

        return compiled_circuit

    compile_device.compile = MethodType(
        compile_with_topology,
        compile_device,
    )

    print(
        "TOPOLOGY_COMPILE_HOOK_TARGET="
        f"{type(compile_device).__module__}.{type(compile_device).__name__}",
        flush=True,
    )
    return device
...
def build_pennylane_device(args, n_qubits: int):
    ...
    1. a Qiskit Aer simulator reproducing a fake backend's noise model,
       basis gates, coupling map, and fixed initial layout, or
    ...
    if args.backend_mode == "qiskit_aer_noisy":
        ...
        fake_class, fake_backend, fake_n_qubits = load_requested_fake_backend(
            fake_provider=fake_provider,
            n_qubits=n_qubits,
            class_name=args.fake_backend_class,
            snapshot_dir=getattr(args, "fake_backend_snapshot_dir", None),
        )

        noise_model = NoiseModel.from_backend(fake_backend)

        initial_layout = parse_initial_layout(
            getattr(args, "initial_layout", None),
            n_qubits=n_qubits,
            backend_n_qubits=fake_n_qubits,
        )
        if getattr(args, "fake_backend_snapshot_dir", None) and initial_layout is None:
            raise ValueError(
                "A frozen FakeAachen run requires an explicit --initial_layout."
            )

        coupling_edge_count = None
        if initial_layout is not None:
            coupling_edge_count = validate_chain_layout(
                fake_backend,
                initial_layout,
            )
        ...
        # from_backend preserves the fake device's noise model, target,
        ...
        # PennyLane-Qiskit 0.44.1's legacy Aer device hard-codes
        # routing_method="none". Set the fixed physical layout and replace
        # only its compile method so Qiskit performs topology-aware routing.
        if initial_layout is not None:
            expected_logical_entanglers = (
                int(args.depth)
                * int(args.n_ansatz_layers)
                * int(n_qubits)
            )
            install_topology_compile_hook(
                dev,
                routing_method=str(args.routing_method),
                initial_layout=initial_layout,
                expected_logical_entanglers=expected_logical_entanglers,
                metadata_path=getattr(args, "transpile_metadata_path", None),
            )
        ...
        metadata = {
            ...
            "topology_constraints_used": initial_layout is not None,
            "initial_layout": (
                None if initial_layout is None else list(initial_layout)
            ),
            "routing_method": (
                None if initial_layout is None else str(args.routing_method)
            ),
            "coupling_edge_count": coupling_edge_count,
            "fake_backend_snapshot_dir": getattr(
                args,
                "fake_backend_snapshot_dir",
                None,
            ),
...
def unpack_quantum_params_np(
    ...
    *,
    ...
    """
    Unpack the flat SPSA quantum-side vector.
    ...
    if encoding in {
        ...
        strength_raw = q_params[
            pos:
            pos + n_encoding_block
        ].reshape(
            12,
            int(depth),
        )
    ...
    if pos != len(q_params):
        raise RuntimeError(
            "Quantum parameter unpack mismatch: "
            f"encoding={encoding}, "
            f"consumed={pos}, "
            f"received={len(q_params)}"
        )
...
def build_direct_window_qnode(
    *,
    ...
    """
    Construct the black-box finite-shot seasonal QNode used by SPSA.

    The upload gate ordering is deliberately identical to the
    validated ideal seasonal models:

        qml.RY(theta)
        qml.RZ(phi)

    so the state operator is
        ...
        for reupload_layer in range(
            ...
            for q in range(
                ...
                # -----------------------------------------------------
                    ...
                    # -------------------------------------------------
                    # Model 5:
                    # learned bounded seasonal azimuth
                    # -------------------------------------------------
            ...
            # ---------------------------------------------------------
            # Existing A=1 trainable ansatz block
            # ---------------------------------------------------------
...
# ============================================================
# Persistent sample-level quantum worker pool
# ============================================================

_WORKER_QNODE = None
_WORKER_N_FEATURES = None


def _worker_identity_index() -> int:
    """Return a stable zero-based worker index within a fresh spawned pool."""
...
def _initialize_quantum_worker(worker_config: Dict[str, Any]):
    """
    Build one independent PennyLane device and QNode inside each process.

    QNodes and Aer simulators are deliberately not shared or pickled. Each
    persistent worker owns one backend for its entire lifetime.
    """
    global _WORKER_QNODE, _WORKER_N_FEATURES

    threads = int(worker_config["aer_threads_per_worker"])
    os.environ["OMP_NUM_THREADS"] = str(threads)
    os.environ["MKL_NUM_THREADS"] = "1"
    os.environ["OPENBLAS_NUM_THREADS"] = "1"
    os.environ["NUMEXPR_NUM_THREADS"] = "1"

    device_args = dict(worker_config["device_args"])
    worker_index = _worker_identity_index() % int(worker_config["num_workers"])

    # Independent worker-level random streams avoid duplicated Aer streams.
    if int(device_args["seed_simulator"]) >= 0:
        device_args["seed_simulator"] = (
            int(device_args["seed_simulator"]) + worker_index
        )
    # Keep the transpiler seed identical across workers so every worker
    # executes the same compiled logical circuit. Only shot/noise streams
    # receive worker-specific simulator seeds.
    device_args["aer_threads"] = threads
    if worker_index != 0:
        device_args["transpile_metadata_path"] = None

    args_namespace = argparse.Namespace(**device_args)
    device, _ = build_pennylane_device(
        args_namespace,
        int(worker_config["n_qubits"]),
    )

    _WORKER_QNODE = build_direct_window_qnode(
        device=device,
        n_qubits=int(worker_config["n_qubits"]),
        depth=int(worker_config["depth"]),
        n_ansatz_layers=int(worker_config["n_ansatz_layers"]),
        angle_factor=float(worker_config["angle_factor"]),
        readout_wires=tuple(worker_config["readout_wires"]),
        shots=int(worker_config["shots"]),
        encoding=str(worker_config["encoding"]),
    )
    _WORKER_N_FEATURES = len(worker_config["readout_wires"])


def _evaluate_quantum_chunk(task):
    """Evaluate one contiguous sample chunk inside a persistent worker."""
    if _WORKER_QNODE is None:
        raise RuntimeError("Quantum worker QNode was not initialized.")

    group, start_index, q_params_np, x_chunk_np = task
    rows = []

    for x_np in x_chunk_np:
        q_out = _WORKER_QNODE(q_params_np, x_np)
        if isinstance(q_out, (tuple, list)):
            values = [float(v) for v in q_out]
        else:
            values = [float(v) for v in np.asarray(q_out).reshape(-1)]

        if len(values) != _WORKER_N_FEATURES:
            raise ValueError(
                f"Worker QNode returned {len(values)} values; "
                f"expected {_WORKER_N_FEATURES}."
            )
        rows.append(values)

    return int(group), int(start_index), np.asarray(rows, dtype=np.float64)


def _split_contiguous_chunks(x_array: np.ndarray, n_chunks: int):
    """Return non-empty contiguous chunks with their starting indices."""
    n_items = int(len(x_array))
    if n_items == 0:
        return []

    n_chunks = max(1, min(int(n_chunks), n_items))
    index_chunks = np.array_split(np.arange(n_items), n_chunks)
    chunks = []
    for indices in index_chunks:
        if len(indices) == 0:
            continue
        start = int(indices[0])
        chunks.append((start, x_array[indices]))
    return chunks


class PersistentQuantumWorkerPool:
    """
    Persistent process pool for independent sample-level QNode evaluations.

    Each worker owns one Aer/PennyLane backend. The parent sends only NumPy
    parameters and sample chunks, then reconstructs outputs in sample order.
    """

    def __init__(self, worker_config: Dict[str, Any]):
        self.worker_config = copy.deepcopy(worker_config)
        self.num_workers = int(worker_config["num_workers"])
        if self.num_workers <= 0:
            raise ValueError("num_workers must be positive for a worker pool.")

        context = mp.get_context("spawn")
        self._executor = ProcessPoolExecutor(
            max_workers=self.num_workers,
            mp_context=context,
            initializer=_initialize_quantum_worker,
            initargs=(self.worker_config,),
        )
        self._closed = False
        atexit.register(self.shutdown)

    def _run_tasks(self, tasks, n_groups: int, group_size: int) -> Tuple[np.ndarray, ...]:
        results = list(
            self._executor.map(
                _evaluate_quantum_chunk,
                tasks,
                chunksize=1,
            )
        )

        outputs = [None] * n_groups
        n_features = len(self.worker_config["readout_wires"])
        for group in range(n_groups):
            outputs[group] = np.empty(
                (group_size, n_features),
                dtype=np.float64,
            )

        for group, start, rows in results:
            outputs[group][start:start + len(rows)] = rows

        return tuple(outputs)

    def evaluate(self, q_params_np: np.ndarray, x_batch_np: np.ndarray) -> np.ndarray:
        chunks = _split_contiguous_chunks(x_batch_np, self.num_workers)
        tasks = [
            (0, start, q_params_np, chunk)
            for start, chunk in chunks
        ]
        return self._run_tasks(tasks, n_groups=1, group_size=len(x_batch_np))[0]

    def evaluate_pair(
        self,
        theta_plus_np: np.ndarray,
        theta_minus_np: np.ndarray,
        x_batch_np: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray]:
        chunks = _split_contiguous_chunks(x_batch_np, self.num_workers)
        tasks = []
        for group, q_params_np in (
            (0, theta_plus_np),
            (1, theta_minus_np),
        ):
            tasks.extend(
                (group, start, q_params_np, chunk)
                for start, chunk in chunks
            )

        plus, minus = self._run_tasks(
            tasks,
            n_groups=2,
            group_size=len(x_batch_np),
        )
        return plus, minus
...
def prepare_worker_pool(
    ...
    """Create a serializable worker configuration and backend metadata."""
    selected_fake_class = args.fake_backend_class
    ...
    if args.backend_mode == "qiskit_aer_noisy":
        ...
        fake_class, fake_backend, fake_n_qubits = load_requested_fake_backend(
            fake_provider=fake_provider,
            n_qubits=n_qubits,
            class_name=selected_fake_class,
            snapshot_dir=args.fake_backend_snapshot_dir,
        )
        selected_fake_class = fake_class
        noise_model = NoiseModel.from_backend(fake_backend)
        initial_layout = parse_initial_layout(
            args.initial_layout,
            n_qubits=n_qubits,
        ...
        if args.fake_backend_snapshot_dir and initial_layout is None:
            raise ValueError(
                "A frozen FakeAachen run requires an explicit --initial_layout."
            )
        coupling_edge_count = None
        ...
        metadata = {
            ...
            "routing_method": (
                None if initial_layout is None else args.routing_method
            ),
            "coupling_edge_count": coupling_edge_count,
            "fake_backend_snapshot_dir": args.fake_backend_snapshot_dir,
            "aer_method": args.aer_method,
            "aer_threads_per_worker": args.aer_threads,
            "num_quantum_workers": args.num_quantum_workers,
            "seed_simulator_base": (
                None if args.seed_simulator < 0 else args.seed_simulator
            ),
        }
    else:
        metadata = {
            "backend_mode": args.backend_mode,
            "pennylane_device": args.qml_device,
            "device_kwargs": parse_json_dict(
                args.device_kwargs_json,
                "--device_kwargs_json",
            ),
            "topology_constraints_used": None,
            "aer_threads_per_worker": args.aer_threads,
    ...
    device_args = {
        ...
        "device_kwargs_json": args.device_kwargs_json,
        "fake_backend_class": selected_fake_class,
        "fake_backend_snapshot_dir": args.fake_backend_snapshot_dir,
        ...
        "transpile_metadata_path": str(
            Path(args.output_dir)
            / args.variant_id
            / f"depth_{args.depth}"
            / f"seed_{args.seed}"
            / "topology_transpile_worker0.json"
        ),
    }

    worker_config = {
        "num_workers": int(args.num_quantum_workers),
        "aer_threads_per_worker": int(args.aer_threads),
        "device_args": device_args,
        "n_qubits": int(n_qubits),
        "depth": int(args.depth),
        "n_ansatz_layers": int(args.n_ansatz_layers),
        "angle_factor": float(args.angle_factor),
        "readout_wires": list(readout_wires),
        "shots": int(args.shots),
        "encoding": str(args.encoding),
    }

    return PersistentQuantumWorkerPool(worker_config), metadata


def process_tree_rss_bytes() -> int:
    """Return aggregate RSS for the parent process and all live descendants."""
    try:
        import psutil
...
def run_worker_pool_benchmark(
    *,
    ...
    """Measure steady-state sample throughput without validation or reports."""
    batch_size = min(int(args.worker_benchmark_batch_size), len(X_train))
    if batch_size <= 0:
        raise ValueError("worker_benchmark_batch_size must be positive.")
    if args.worker_benchmark_repeats <= 0:
        raise ValueError("worker_benchmark_repeats must be positive.")

    x_batch = X_train[:batch_size]
    theta_center = model.pack_quantum_params().detach().clone()
    rng = np.random.default_rng(args.spsa_seed)
    delta_np = rng.choice(
        np.array([-1.0, 1.0]),
        size=tuple(theta_center.shape),
    )
    delta = torch.tensor(delta_np, dtype=torch.float64)
    theta_plus = theta_center + args.spsa_c * delta
    theta_minus = theta_center - args.spsa_c * delta

    # Force all persistent workers to start and build their QNodes before the
    # timed region. At most one warm-up sample is assigned per worker.
    warmup_parallelism = (
        args.num_quantum_workers
        if args.num_quantum_workers > 0
        else 1
    )
    warmup_count = min(warmup_parallelism, len(X_train))
    warmup_start = time.perf_counter()
    _ = model.quantum_features(
        X_train[:warmup_count],
        q_params=theta_center,
    )
    warmup_wall_sec = time.perf_counter() - warmup_start
    warmup_calls = warmup_count

    model.qnode_call_count = 0
    repeat_times = []
    feature_means = []

    benchmark_start = time.perf_counter()
    for _ in range(args.worker_benchmark_repeats):
        repeat_start = time.perf_counter()
        z_plus, z_minus = model.quantum_feature_pair(
            x_batch,
            theta_plus,
            theta_minus,
        )
        ...
        feature_means.append(
            float(torch.cat((z_plus, z_minus), dim=0).mean().item())
        )
    benchmark_wall_sec = time.perf_counter() - benchmark_start

    expected_calls = (
        2 * batch_size * int(args.worker_benchmark_repeats)
    )
    if model.qnode_call_count != expected_calls:
        raise RuntimeError(
            f"Worker benchmark expected {expected_calls} calls, "
            f"observed {model.qnode_call_count}."
        )

    aggregate_rss_bytes = process_tree_rss_bytes()
    calls_per_second = expected_calls / benchmark_wall_sec
    seconds_per_call = benchmark_wall_sec / expected_calls

    execution_strategy = (
        "persistent_process_pool"
        if args.num_quantum_workers > 0
        else "in_process_serial_samples"
    )
    effective_cpu_slots = (
        args.num_quantum_workers * args.aer_threads
        if args.num_quantum_workers > 0
        else args.aer_threads
    )

    row = {
        "execution_strategy": execution_strategy,
        "num_quantum_workers": int(args.num_quantum_workers),
        "aer_threads_per_worker": int(args.aer_threads),
        "effective_cpu_slots": int(effective_cpu_slots),
        "depth": int(args.depth),
        "shots": int(args.shots),
        "batch_size": int(batch_size),
        "benchmark_repeats": int(args.worker_benchmark_repeats),
        "warmup_calls": int(warmup_calls),
        "warmup_wall_sec": float(warmup_wall_sec),
        "timed_qnode_calls": int(expected_calls),
        "timed_wall_sec": float(benchmark_wall_sec),
        "qnode_calls_per_sec": float(calls_per_second),
        "seconds_per_qnode_call": float(seconds_per_call),
        "mean_repeat_wall_sec": float(np.mean(repeat_times)),
        "min_repeat_wall_sec": float(np.min(repeat_times)),
        "max_repeat_wall_sec": float(np.max(repeat_times)),
        "aggregate_process_tree_rss_bytes": int(aggregate_rss_bytes),
        "aggregate_process_tree_rss_gb": (
            aggregate_rss_bytes / (1024 ** 3)
            if aggregate_rss_bytes >= 0
            else np.nan
        ),
        "feature_mean_checksum": float(np.mean(feature_means)),
    }

    csv_path = out_dir / "worker_pool_benchmark.csv"
    json_path = out_dir / "worker_pool_benchmark.json"
    pd.DataFrame([row]).to_csv(csv_path, index=False)
    with open(json_path, "w") as file:
        json.dump(row, file, indent=2)

    print("\n" + "=" * 100)
    print("WORKER POOL BENCHMARK")
    print("=" * 100)
    ...
    if model.quantum_executor is not None:
        model.quantum_executor.shutdown()
...
class DirectWindowQNNNoisy(nn.Module):
    ...
    def __init__(
        self,
        *,
        quantum_circuit: Optional[Callable[[np.ndarray, np.ndarray], Any]],
        quantum_executor: Optional[PersistentQuantumWorkerPool],
        n_qubits: int,
        depth: int,
        n_ansatz_layers: int,
        readout_wires: Sequence[int],
        ...
        if self.quantum_circuit is None and self.quantum_executor is None:
            raise ValueError("A quantum circuit or quantum executor is required.")
        ...
        if self.encoding not in allowed_encodings:
            raise ValueError(
                f"Unsupported seasonal encoding: {self.encoding}"
            )
        ...
        # -------------------------------------------------------------
        # Seasonal encoder raw parameters.
        #
        # All start at exactly zero:
        #
        # strength = 1 + tanh(0) = 1
        # shift    = pi/2 * tanh(0) = 0
        # dphi     = pi/12 * tanh(0) = 0
        #
        # Therefore both learned models are exactly nested over the
        # fixed seasonal model at initialization.
        # -------------------------------------------------------------
        ...
        if self.encoding in {
            ...
            self.encoding_strength_raw = nn.Parameter(
                torch.zeros(
                    12,
                    self.depth,
                    dtype=torch.float64,
            ...
            self.encoding_shift_raw = nn.Parameter(
                torch.zeros(
                    12,
                    self.depth,
                    dtype=torch.float64,
        ...
        if (
            ...
            self.encoding_phase_raw = nn.Parameter(
                torch.zeros(
                    12,
                    self.depth,
                    dtype=torch.float64,
    ...
    @property
    def q_params(self):
        return self.pack_quantum_params()
    ...
    @property
    def ansatz_parameter_count(self):
        return int(
            self.q_weights.numel()
        )

    @property
    def encoding_parameter_count(self):
        ...
        if (
            ...
            count += int(
                self.encoding_strength_raw.numel()
            )
    ...
    def quantum_parameter_count(self):
        ...
        return (
            self.ansatz_parameter_count
            +
            self.encoding_parameter_count
        )

    def pack_quantum_params(self):
    ...
    def set_quantum_params(
        ...
        if theta.numel() != expected:
            ...
            raise ValueError(
                "Packed quantum parameter size mismatch: "
        ...
        if pos != theta.numel():
            ...
            raise RuntimeError(
                "Internal packed quantum parameter "
                "assignment mismatch."
            )
    ...
    def _validate_quantum_batch(self, x_batch: Tensor):
        valid_shape = (
            x_batch.ndim == 3
            and
            x_batch.shape[1] == self.n_qubits
            and
            x_batch.shape[2] == 2
        )
        ...
        if not valid_shape:
            raise ValueError(
                "Seasonal x_batch must have shape "
                f"(batch_size, {self.n_qubits}, 2); "
    ...
    def quantum_features(self, x_batch: Tensor, q_params: Optional[Tensor] = None) -> Tensor:
        self._validate_quantum_batch(x_batch)
        q_params = (
            self.pack_quantum_params()
            if q_params is None
            else q_params
        )
        q_params_np = self._to_numpy_safe(q_params)
        x_batch_np = self._to_numpy_safe(x_batch)

        if self.quantum_executor is not None:
            output_np = self.quantum_executor.evaluate(
                q_params_np,
                x_batch_np,
            )
            self.qnode_call_count += int(len(x_batch))
            return torch.tensor(output_np, dtype=torch.float64)

        outputs = []
        for x_np in x_batch_np:
            q_out = self.quantum_circuit(q_params_np, x_np)
    ...
    def quantum_feature_pair(
        ...
        if self.quantum_executor is None:
            z_plus = self.quantum_features(x_batch, q_params=theta_plus)
            z_minus = self.quantum_features(x_batch, q_params=theta_minus)
            return z_plus, z_minus

        plus_np, minus_np = self.quantum_executor.evaluate_pair(
            self._to_numpy_safe(theta_plus),
            self._to_numpy_safe(theta_minus),
            self._to_numpy_safe(x_batch),
        )
        self.qnode_call_count += 2 * int(len(x_batch))
        return (
            torch.tensor(plus_np, dtype=torch.float64),
            torch.tensor(minus_np, dtype=torch.float64),
        )
...
def two_pass_spsa_adam_step(
    *,
    ...
    head_optimizer.step()
    with torch.no_grad():
        model.set_quantum_params(theta_new)
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory containing a frozen conf_aachen.json and "
            "props_aachen.json snapshot."
        ),
    )
    parser.add_argument(
        "--initial_layout",
        type=str,
        default=None,
        help="Comma-separated logical-to-physical qubit layout.",
    )
    parser.add_argument(
        "--routing_method",
        choices=["sabre", "basic", "lookahead", "stochastic", "none"],
        default="sabre",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "0 uses the original in-process QNode loop; a positive value "
            "uses that many persistent sample-level worker processes."
        ),
    )
    parser.add_argument(
        "--worker_benchmark_only",
        action="store_true",
        help="Benchmark pooled QNode throughput and exit before training.",
    )
    parser.add_argument(
        "--worker_benchmark_batch_size",
        type=int,
        default=16,
    )
    parser.add_argument(
        "--worker_benchmark_repeats",
        type=int,
        default=2,
    )
    ...
    parser.add_argument("--variant_id", type=str, default="direct14_noisy_seasonal_two_pass")
    ...
    if args.num_quantum_workers < 0:
        raise ValueError("num_quantum_workers cannot be negative.")
    if args.num_quantum_workers > 0 and args.aer_threads <= 0:
        raise ValueError("aer_threads must be positive for worker execution.")

    if (
        args.wrap_quantum_angles
        and
        args.encoding in {
            "learnable_seasonal_cdf",
            "learnable_seasonal_cdf_rz",
        ...
        raise ValueError(
            "--wrap_quantum_angles cannot be used for a packed vector "
            "containing learned seasonal raw parameters."
        )
    ...
    print("Seasonal CDF reference size:", data["cdf_reference_size"])
    print("Seasonal CDF reference end:", data["cdf_reference_end"])
    ...
    if args.num_quantum_workers > 0:
        quantum_executor, backend_metadata = prepare_worker_pool(
            args,
            n_qubits,
            readout_wires,
        )
    ...
    config.update({
        ...
        "ansatz_parameter_count": model.ansatz_parameter_count,
        "encoding_parameter_count": model.encoding_parameter_count,
        "spsa_quantum_parameter_count": model.quantum_parameter_count,
    ...
    if args.worker_benchmark_only:
        run_worker_pool_benchmark(
            model=model,
            X_train=X_train,
            args=args,
            out_dir=out_dir,
        )
        print("\nWorker benchmark completed; training was intentionally skipped.")
        return
